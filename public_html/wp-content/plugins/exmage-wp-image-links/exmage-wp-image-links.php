...
define( 'EXMAGE_WP_IMAGE_LINKS_VERSION', '1.0.12' );
			...
			if ( $attachment_id ) {
				...
				if ( $attachment && $attachment->post_type === 'attachment' ) {
					...
					if ( $to_external ) {
						...
						if ( $to_url ) {
							$image_size         = function_exists( 'wp_getimagesize' ) ? wp_getimagesize( $to_url ) : getimagesize( $to_url );
							...
							if ( $image_size !== false ) {
								...
								$request = wp_safe_remote_get( $to_url );
		...
		private static function update_attachment_metadata( $attachment_id, $url, $is_ali_cdn, $width, $height ) {
			...
			if ( ! empty( $pathinfo['extension'] ) ) {
				...
				foreach ( $common_sizes as $size_name => $size_width ) {
					...
					if ( ! $is_ali_cdn || wp_remote_retrieve_response_code( wp_safe_remote_get( $size_url ) ) !== 200 ) {
						$size_url = $url;
					}
